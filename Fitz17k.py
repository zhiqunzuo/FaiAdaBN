...
class Fitz17k(BaseDataset):
    def __init__(self, dataframe, path_to_pickles, sens_name, sens_classes, transform):
        ...
        #one_hot_labels = np.array(pd.get_dummies(self.dataframe["nine_partition_label"])).astype(np.int8)
        self.Y = self.dataframe["nine_partition_label"].astype("category").cat.codes
