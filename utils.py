...
def load_weights(net, weights='imagenet', pth='./pretrained_weights/exp_0_final.pkl'):
    ...
    for name, _ in net.state_dict().items():
        ...
        if not ('ins' in name or 'running' in name or 'batches' in name or 'bn' in name or 'shortcut1' in name or 'conv1.1' in name or 'bias' in name):
    ...
    for i in range(len(pretrain_net_keys)):
        print("net key = {}, pretrain net key = {}".format(net_keys[i], pretrain_net_keys[i]))
